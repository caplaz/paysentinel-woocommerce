...
class PaySentinel_Alerts {
	...
	public function validate_license( $license_key ) {
		...
		$license_obj = new PaySentinel_License();
		$result      = $license_obj->validate_license( $license_key );

		if ( $result['valid'] ) {
			return array(
				'valid'   => true,
				'message' => __( 'License validated successfully', 'paysentinel' ),
				'expires' => isset( $result['data']['expiration_ts'] ) ? $result['data']['expiration_ts'] : null,
			);
		} else {
			return array(
				'valid'   => false,
				'message' => isset( $result['message'] ) ? $result['message'] : __( 'Invalid license key', 'paysentinel' ),
			);
		}
	}
