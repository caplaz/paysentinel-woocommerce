...
class WC_Payment_Monitor_Failure_Simulator {
	...
	public function simulate_failure_for_order( $order_id_or_object, $scenario_key ) {
		...
		$order->update_status(
			'failed',
			sprintf(
				__( '[SIMULATED FAILURE] %1$s (Error Code: %2$s)', 'wc-payment-monitor' ),
				$scenario['message'],
		...
		return array(
			'success' => true,
			'message' => sprintf(
				__( 'Successfully simulated %1$s failure for order #%2$d.', 'wc-payment-monitor' ),
				$scenario['name'],
				$order->get_id()
			),
		);
	...
	public function clear_simulated_failures() {
		...
		return array(
			...
			'message'              => sprintf(
				__( 'Cleared %1$d simulated orders and %2$d transaction records.', 'wc-payment-monitor' ),
				$deleted_orders,
