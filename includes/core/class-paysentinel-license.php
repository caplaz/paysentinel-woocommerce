...
class PaySentinel_License {
	 ...
	 * Option names
	 */
	public const OPTION_LICENSE_KEY            = 'paysentinel_license_key';
	public const OPTION_LICENSE_STATUS         = 'paysentinel_license_status';
	public const OPTION_LICENSE_DATA           = 'paysentinel_license_data';
	public const OPTION_LAST_CHECK             = 'paysentinel_license_last_check';
	public const OPTION_SITE_REGISTERED        = 'paysentinel_site_registered';
	public const OPTION_SITE_REGISTRATION_DATA = 'paysentinel_site_registration_data';
	public const OPTION_SITE_SECRET            = 'paysentinel_site_secret';
	...
	public function activate_license( $license_key, $site_url = '' ) {
		...
		if ( empty( $license_key ) ) {
			return array(
				'success' => false,
				'message' => __( 'License key is required', 'paysentinel' ),
				'data'    => null,
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'License activation failed: %s', 'paysentinel' ),
		...
		if ( empty( $site_secret ) ) {
			return array(
				'success'    => false,
				'message'    => __( 'Site secret not received from server', 'paysentinel' ),
				'data'       => $data,
				'debug_info' => $debug_info,
			);
		}
		...
		return array(
			...
			'message'         => isset( $data['message'] ) ? $data['message'] : __( 'License activated successfully', 'paysentinel' ),
			'data'            => $data,
			'debug_info'      => $debug_info,
		);
	}
	...
	public function validate_license( $license_key, $site_url = '', $site_secret = null ) {
		...
		if ( empty( $site_secret ) ) {
			return array(
				'valid'   => false,
				'message' => __( 'Site not activated. Please activate your license first.', 'paysentinel' ),
				'data'    => null,
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return array(
				...
				'message' => sprintf(
					/* translators: %s: error message */
					__( 'License validation failed: %s', 'paysentinel' ),
					$response->get_error_message()
				),
				'data'    => null,
			);
		...
		return array(
			'valid'      => $is_valid,
			'message'    => isset( $data['message'] ) ? $data['message'] : ( $is_valid ? __( 'License is valid', 'paysentinel' ) : __( 'License is invalid', 'paysentinel' ) ),
			'data'       => $data,
			'debug_info' => $debug_info,
		);
	}
	...
	private function register_site_with_license( $license_key ) {
		...
		return array(
			...
			'reason'  => isset( $result['message'] ) ? $result['message'] : __( 'Site registration completed', 'paysentinel' ),
		);
	}
	...
	public function save_and_validate_license( $license_key ) {
		...
		if ( empty( $site_secret ) ) {
			return array(
				'valid'   => false,
				'message' => __( 'Site secret not received from activation. Please try again.', 'paysentinel' ),
				'data'    => $activation_result['data'],
			);
		}
		...
		if ( $validation_result['valid'] && $site_registered ) {
			...
			$message = __( 'License is valid, but site registration is pending.', 'paysentinel' );
		} else {
	...
	public function sync_license() {
		...
		if ( ! $this->is_site_registered() ) {
			return new WP_Error(
				'site_not_registered',
				__( 'Site must be registered before syncing license data.', 'paysentinel' )
			);
		}
		...
		if ( 200 === $response_code ) {
			...
			return array(
				'success' => true,
				'message' => __( 'License synced successfully', 'paysentinel' ),
				'data'    => $data,
			);
		} elseif ( 401 === $response_code ) {
			return new WP_Error(
				'unauthorized',
				__( 'Invalid HMAC signature. Please re-validate your license.', 'paysentinel' )
			);
		} elseif ( 403 === $response_code ) {
			update_option( self::OPTION_LICENSE_STATUS, 'invalid' );
			return new WP_Error(
				'forbidden',
				__( 'License is invalid or expired.', 'paysentinel' )
			);
		} else {
			return new WP_Error(
				'sync_failed',
				sprintf(
					...
					__( 'License sync failed with HTTP status %d', 'paysentinel' ),
					$response_code
				)
			);
		}
		...
		if ( 'unknown' === $status ) {
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					...
					printf(
						...
						esc_html__( 'Please enter your license key to activate the plugin. %s', 'paysentinel' ),
						'<a href="' . esc_url( admin_url( 'admin.php?page=paysentinel-settings' ) ) . '">' . esc_html__( 'Go to Settings', 'paysentinel' ) . '</a>'
					);
					?>
				</p>
			</div>
			...
			<div class="notice notice-error is-dismissible">
				<p>
					...
					printf(
						/* translators: %s: settings page link */
						esc_html__( 'Your license key is invalid or expired. Please check your license key and try again. %s', 'paysentinel' ),
						'<a href="' . esc_url( admin_url( 'admin.php?page=paysentinel-settings' ) ) . '">' . esc_html__( 'Go to Settings', 'paysentinel' ) . '</a>'
					);
					?>
				</p>
				<?php if ( ! empty( $debug_info ) ) : ?>
					<p
						...
						<strong><?php esc_html_e( 'Debug Information:', 'paysentinel' ); ?></strong><br>
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					<strong><?php esc_html_e( 'PaySentinel - Payment Monitor for WooCommerce:', 'paysentinel' ); ?></strong>
					<?php esc_html_e( 'Your license is valid, but this site is not registered. Alerts will not be sent until the site is registered with your license.', 'paysentinel' ); ?>
				</p>
			</div>
			<?php
		}
	...
	private function get_user_friendly_error_message( $response_code, $data ) {
		...
		if ( isset( $data['error'] ) ) {
			switch ( $data['error'] ) {
				case 'License not found':
					return __( 'Invalid license key. Please check your license key and try again.', 'paysentinel' );

				case 'License expired':
					return __( 'Your license has expired. Please renew your license to continue using premium features.', 'paysentinel' );

				case 'License suspended':
					return __( 'Your license has been suspended. Please contact support for assistance.', 'paysentinel' );

				case 'Too many activations':
					return __( 'This license key has reached its activation limit. Please contact support to increase your limit.', 'paysentinel' );

				case 'Invalid domain':
					return __( 'This license key is not valid for this domain. Please check your license restrictions.', 'paysentinel' );
		...
		switch ( $response_code ) {
			...
			case 401:
				return __( 'License authentication failed. Please check your license key.', 'paysentinel' );

			case 403:
				return __( 'License access denied. Your license may be invalid or expired.', 'paysentinel' );

			case 404:
				return __( 'License server not found. Please try again later or contact support.', 'paysentinel' );

			case 429:
				return __( 'Too many license validation attempts. Please wait a few minutes and try again.', 'paysentinel' );

			case 500:
			case 502:
			case 503:
			case 504:
				return __( 'License server is temporarily unavailable. Please try again later.', 'paysentinel' );

			default:
				return __( 'Unable to validate license. Please check your internet connection and try again.', 'paysentinel' );
		}
	}
	...
	public function deactivate_license() {
		...
		return array(
			'valid'   => true,
			'message' => __( 'License deactivated successfully', 'paysentinel' ),
		);
	}
	...
	public function make_authenticated_request( $endpoint, $method = 'POST', $body = array(), $include_site_url = true ) {
		...
		if ( empty( $license_key ) || empty( $site_secret ) ) {
			return new WP_Error(
				...
				__( 'Authentication details missing. Please re-validate your license.', 'paysentinel' )
			);
		}
	...
	private function make_authenticated_request_with_secret( $endpoint, $method, $body, $site_secret, $license_key, $include_site_url = true ) {
		...
		if ( ! empty( $body ) ) {
			...
			PaySentinel_Security::recursive_ksort( $body_array );
