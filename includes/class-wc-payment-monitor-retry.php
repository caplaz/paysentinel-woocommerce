...
class WC_Payment_Monitor_Retry {
    ...
    private function process_payment_retry($order, $transaction) {
        try {
            ...
            $order->add_order_note(sprintf(
                __('Attempting automatic payment retry %d of %d', 'wc-payment-monitor'),
    ...
    private function handle_successful_retry($order, $transaction, $retry_result) {
        ...
        $order->add_order_note(sprintf(
            __('Payment retry successful on attempt %d. Transaction ID: %s', 'wc-payment-monitor'),
            $transaction->retry_count + 1,
    ...
    private function handle_failed_retry($order, $transaction, $retry_result, $retry_count) {
        ...
        // Add order note
        $order->add_order_note(sprintf(
            __('Payment retry %d failed: %s', 'wc-payment-monitor'),
            $retry_count,
    ...
    public function send_retry_success_email($order) {
        ...
        $subject = sprintf(
            __('[%s] Payment Successful - Order #%s', 'wc-payment-monitor'),
            get_bloginfo('name'),
