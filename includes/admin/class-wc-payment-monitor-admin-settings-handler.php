...
class WC_Payment_Monitor_Admin_Settings_Handler {
	 ...
	 * Constructor
	 *
	 * @param WC_Payment_Monitor_Security $security Security instance.
		...
		<p class="description">
			...
			if ( $is_locked ) {
				...
				if ( $quota && isset( $quota['sms_remaining'], $quota['sms_limit'] ) ) {
					printf(
						__( 'SMS alerts delivered via PaySentinel servers. Quota: %1$d/%2$d remaining this month.', 'wc-payment-monitor' ),
						$quota['sms_remaining'],
						$quota['sms_limit']
					);
				} else {
