...
class PaySentinel_Admin_Settings_Handler {
	...
	private function register_settings_sections() {
		...
		add_settings_section(
			...
			__( 'Notification Settings', 'paysentinel' ),
	...
	private function register_settings_fields() {
		...
		add_settings_field(
			'alert_email',
			__( 'Alert Email Address', 'paysentinel' ),
	...
	public function render_notifications_section() {
		echo '<p>' . esc_html__( 'Configure how you receive alerts when issues are detected.', 'paysentinel' ) . '</p>';
	}
	...
	public function render_field_alert_email() {
		...
		<input type="email" name="paysentinel_options[alert_email]" value="<?php echo esc_attr( $email ); ?>"
			class="regular-text" />
		<p class="description">
			<?php esc_html_e( 'Email address to receive payment failure alerts (Free tier - local delivery).', 'paysentinel' ); ?>
		</p>
		<?php
	}
	...
	public function render_field_alert_slack_workspace() {
		$slack     = get_option( 'paysentinel_slack_workspace', '' );
		$tier      = $this->license->get_license_tier();
		...
		$timestamp   = time();
		$signature   = PaySentinel_Security::generate_hmac_signature( '', $timestamp, $site_secret );

		$return_url            = admin_url( 'admin.php?page=paysentinel-settings&tab=notifications&slack_auth=1' );
		$status_info           = '';
		$is_actually_connected = false;
		$connection_error      = '';

		if ( ! $is_locked && ! empty( $slack ) ) {
			$status_endpoint = add_query_arg( 'integration_id', $slack, PaySentinel_License::SAAS_URL . '/api/integrations/slack/status' );
			...
			if ( is_wp_error( $status_response ) ) {
				...
				if ( ! empty( $status_data['channel_name'] ) && ! in_array( strtolower( $status_data['channel_name'] ), array( 'connected', 'active', 'ok' ), true ) ) {
					$status_info = sprintf( ' <span style="color: #666; font-weight: normal;">(%s)</span>', esc_html( $status_data['channel_name'] ) );
				} elseif ( ! empty( $status_data['channel'] ) && ! in_array( strtolower( $status_data['channel'] ), array( 'connected', 'active', 'ok' ), true ) ) {
				...
				if ( 401 === $response_code ) {
					$connection_error = __( 'Authentication failed - please reconnect', 'paysentinel' );
				} elseif ( 404 === $response_code ) {
					$connection_error = __( 'Integration not found - please reconnect', 'paysentinel' );
				} else {
					$connection_error = sprintf( __( 'Status check failed (HTTP %d)', 'paysentinel' ), $response_code );
				}
			}
		}
		...
		$connect_url = add_query_arg(
			...
			PaySentinel_License::SAAS_URL . '/api/integrations/slack/connect'
		);
		...
		<div class="slack-integration-container">
			...
			<div style="margin-bottom: 10px; display: flex; align-items: center; gap: 15px;">
				<?php if ( $is_locked ) : ?>
					<div
						...
						<strong><?php esc_html_e( 'Locked', 'paysentinel' ); ?></strong>
					</div>
				<?php elseif ( $is_actually_connected ) : ?>
					<div
						...
						<strong><?php esc_html_e( 'Connected', 'paysentinel' ); ?></strong>
				...
				<?php elseif ( ! empty( $slack ) && ! empty( $connection_error ) ) : ?>
					<div
						...
						<strong><?php esc_html_e( 'Connection Issue', 'paysentinel' ); ?></strong>
						<span
				...
				<?php elseif ( ! empty( $slack ) ) : ?>
					<div
						...
						<strong><?php esc_html_e( 'Status Unknown', 'paysentinel' ); ?></strong>
					</div>
				<?php else : ?>
					<div
						...
						<strong><?php esc_html_e( 'Not Connected', 'paysentinel' ); ?></strong>
					</div>
				<?php endif; ?>
				...
				<?php if ( ! $is_locked ) : ?>
					<a href="<?php echo esc_url( $connect_url ); ?>"
						...
						if ( ! empty( $slack ) && ! empty( $connection_error ) ) {
							esc_html_e( 'Reconnect', 'paysentinel' );
						} elseif ( ! empty( $slack ) ) {
							esc_html_e( 'Update / Reconnect', 'paysentinel' );
						} else {
							esc_html_e( 'Connect Slack Workspace', 'paysentinel' );
						}
						?>
					</a>
					<?php if ( empty( $slack ) ) : ?>
			...
			<p class="description" style="margin-bottom: 15px;">
				...
				if ( $is_locked ) {
					printf(
						__( 'Slack notifications delivered via PaySentinel. <strong>Requires Pro plan or higher.</strong> <a href="%s" target="_blank">Upgrade Now</a>', 'paysentinel' ),
						PaySentinel_License::SAAS_URL . '/plans'
					);
				} elseif ( $is_actually_connected ) {
					esc_html_e( 'Your Slack workspace is successfully connected. Alerts will be delivered to the channel shown above.', 'paysentinel' );
				} elseif ( ! empty( $slack ) && ! empty( $connection_error ) ) {
					esc_html_e( 'There is an issue with your Slack integration. Please reconnect to restore alert delivery.', 'paysentinel' );
				} elseif ( ! empty( $slack ) ) {
					esc_html_e( 'Unable to verify Slack connection status. If alerts are not being delivered, try reconnecting.', 'paysentinel' );
				} else {
					esc_html_e( 'Authorize PaySentinel to send alerts to your Slack workspace. Manual entry is disabled for security.', 'paysentinel' );
				}
				?>
			</p>
			...
			<?php if ( ! $is_locked && $is_actually_connected ) : ?>
				<div style="display: flex; align-items: center; gap: 10px;">
					...
					<a href="<?php echo esc_url( $disconnect_url ); ?>" class="submitdelete deletion"
						...
						<?php esc_html_e( 'Disconnect Integration', 'paysentinel' ); ?>
					</a>
				</div>
			...
			<?php if ( ! $is_locked ) : ?>
				<script>
					jQuery(document).ready(function ($) {
						// Test Alert Handler
						$('#paysentinel-slack-test').on('click', function (e) {
							e.preventDefault();
							var $btn = $(this);
							$btn.prop('disabled', true).text('<?php esc_attr_e( 'Sending...', 'paysentinel' ); ?>');

							$.post(ajaxurl, {
								action: 'paysentinel_slack_test',
								nonce: '<?php echo wp_create_nonce( 'paysentinel_admin_nonce' ); ?>'
							}, function (response) {
								if (response.success) {
								...
								$btn.prop('disabled', false).html('<span class="dashicons dashicons-email-alt" style="font-size: 16px; vertical-align: middle;"></span> <?php esc_attr_e( 'Send Test Alert', 'paysentinel' ); ?>');
							});
						});

						// Sync Handler
						$('#paysentinel-slack-sync').on('click', function (e) {
							e.preventDefault();
							var $btn = $(this);
							var originalHtml = $btn.html();
							$btn.prop('disabled', true).text('<?php esc_attr_e( 'Syncing...', 'paysentinel' ); ?>');

							$.post(ajaxurl, {
								action: 'paysentinel_sync_integrations',
								nonce: '<?php echo wp_create_nonce( 'paysentinel_admin_nonce' ); ?>'
							}, function (response) {
								if (response.success) {
									alert(response.data.message);
