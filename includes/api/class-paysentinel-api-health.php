	...
	public function get_all_gateway_health( $request ) {
		...
		if ( ( '30day' === $backend_period || '90day' === $backend_period ) && ! in_array( $tier, array( 'pro', 'agency' ), true ) ) {
			return $this->get_error_response(
				...
				__( 'Extended analytics history is only available in PRO and Agency plans.', 'paysentinel' ),
				403
			);
		}
		...
		if ( ! in_array( $backend_period, $valid_periods, true ) ) {
			return $this->get_error_response(
				'invalid_period',
				__( 'Invalid health period. Must be one of: 24h, 7d, 30d, 90d', 'paysentinel' ),
				400
			);
		}
		...
		try {
			...
			foreach ( $gateways as $gateway ) {
				...
				if ( $health ) {
					...
					$item = array(
						...
						'gateway_name'            => PaySentinel::get_friendly_gateway_name( $gateway_id ),
			...
			return $this->get_error_response(
				...
				__( 'Failed to retrieve gateway health data', 'paysentinel' ),
				500
			);
		}
	}
	...
	public function get_gateway_health( $request ) {
		...
		try {
			...
			if ( ! isset( $gateways[ $gateway_id ] ) ) {
				return $this->get_error_response(
					'gateway_not_found',
					__( 'Payment gateway not found', 'paysentinel' ),
					404
				);
			}
			...
			if ( ! $health ) {
				return $this->get_error_response(
					...
					__( 'No health data available for this gateway', 'paysentinel' ),
					404
				);
			}
	...
	public function get_gateway_health_history( $request ) {
		...
		try {
			...
			return $this->get_error_response(
				...
				__( 'Failed to retrieve gateway health history', 'paysentinel' ),
				500
			);
		}
	}
	...
	public function recalculate_all_health( $request ) {
		try {
			...
			return $this->get_success_response(
				array(
					...
					'message'        => sprintf(
						...
						__( 'Recalculated health for %d gateway(s)', 'paysentinel' ),
						$recalculated
					),
			...
			return $this->get_error_response(
				...
				__( 'Failed to recalculate gateway health', 'paysentinel' ),
				500
			);
		}
	}
