...
class WC_Payment_Monitor_Analytics_Pro {
	...
	public function get_comparative_analytics( $gateway_id ) {
		if ( ! $this->is_pro_analytics_available() ) {
			return array(
				...
				'message' => __( 'Comparative analytics is a PRO feature', 'wc-payment-monitor' ),
			);
		}
	...
	public function get_failure_pattern_analysis( $gateway_id, $days = 30 ) {
		if ( ! $this->is_pro_analytics_available() ) {
			return array(
				'error'   => 'pro_feature_required',
				'message' => __( 'Failure pattern analysis is a PRO feature', 'wc-payment-monitor' ),
			);
		}
	...
	public function get_advanced_metrics_summary() {
		if ( ! $this->is_pro_analytics_available() ) {
			return array(
				'error'   => 'pro_feature_required',
				'message' => __( 'Advanced metrics summary is a PRO feature', 'wc-payment-monitor' ),
			);
		}
	...
	public function get_extended_history( $gateway_id, $days = 90 ) {
		if ( ! $this->is_pro_analytics_available() ) {
			return array(
				'error'   => 'pro_feature_required',
				'message' => __( 'Extended history is a PRO feature', 'wc-payment-monitor' ),
			);
		}
	...
	public function get_gateway_comparison() {
		if ( ! $this->is_pro_analytics_available() ) {
			return array(
				'error'   => 'pro_feature_required',
				'message' => __( 'Gateway comparison is a PRO feature', 'wc-payment-monitor' ),
			);
		}
