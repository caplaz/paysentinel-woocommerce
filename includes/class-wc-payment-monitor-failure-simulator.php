...
class WC_Payment_Monitor_Failure_Simulator
{
    /**
     * Database instance
     */
    ...
    private function init_hooks()
    {
        ...
        if ($this->is_test_mode_enabled()) {
            add_filter('woocommerce_payment_successful_result', [$this, 'maybe_simulate_failure'], 10, 2);
            add_action('woocommerce_checkout_order_processed', [$this, 'maybe_simulate_processing_failure'], 10, 3);
        }
    }
    ...
    public function maybe_simulate_failure($result, $order_id)
    {
        ...
        return [
            ...
            'messages' => sprintf(
                '<strong>%s:</strong> %s',
                __('Payment Failed (Test Mode)', 'wc-payment-monitor'),
                $scenario['message']
            ),
        ];
    }
    ...
    private function get_or_create_test_product()
    {
        ...
        $existing = get_posts([
            'post_type'      => 'product',
            'posts_per_page' => 1,
            'meta_key'       => '_wc_payment_monitor_test_product',
            'meta_value'     => '1',
            'post_status'    => 'publish',
        ]);
        ...
        update_post_meta($product_id, '_wc_payment_monitor_test_product', '1');
     ...
     */
    public function generate_bulk_failures($count, $gateway_id = null, $scenarios = [])
    {
        $results = [
            'success'      => 0,
            'failed'       => 0,
    ...
    public function clear_simulated_failures()
    {
        ...
        $order_ids = get_posts([
            ...
            'meta_query'     => [
                [
                    'key'     => '_wc_payment_monitor_simulated_failure',
                    'compare' => 'EXISTS',
                ],
            ],
            'fields'         => 'ids',
        ...
        return [
            ...
            'message'              => sprintf(
                __('Cleared %d simulated orders and %d transaction records.', 'wc-payment-monitor'),
                $deleted_orders,
