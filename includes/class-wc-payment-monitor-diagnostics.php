...
class WC_Payment_Monitor_Diagnostics
{
        ...
        foreach ($connectivity_results['results'] as $gateway_id => $status) {
            ...
            if ('offline' === $status['status']) {
                ...
                $results['issues'][] = sprintf(
                    __('Gateway %s is offline: %s', 'wc-payment-monitor'),
                    $gateway_id,
                    $status['message']
                );
            }
            ...
            if (isset($health_data['24hour']['success_rate']) && $health_data['24hour']['success_rate'] < 90) {
                $results['status'] = 'warning';
                $results['issues'][] = sprintf(
                    __('Gateway %s has low success rate: %.1f%%', 'wc-payment-monitor'),
                    $gateway_id,
    ...
    public function reset_gateway_health($gateway_id = '')
    {
        ...
        if (empty($gateway_id)) {
            ...
            $deleted = $wpdb->delete($table_name, ['gateway_id' => $gateway_id]);
    ...
    public function archive_old_transactions($days = 90)
    {
        ...
        return [
            ...
            'message' => sprintf(
                __('Archived (deleted) %d successful transactions older than %d days.', 'wc-payment-monitor'),
                intval($deleted),
                $days
            ),
        ];
