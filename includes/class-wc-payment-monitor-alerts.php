...
class WC_Payment_Monitor_Alerts {
    ...
    private function create_alert_message($alert_data) {
        ...
        $message = sprintf(
            __('Payment gateway "%s" success rate has dropped to %s%% in the last %s. %d out of %d transactions failed.', 'wc-payment-monitor'),
            $gateway_name,
            $success_rate,
    ...
    private function send_email_notification($alert_data, $email_address) {
        $subject = sprintf(
            __('[%s] Payment Gateway Alert - %s', 'wc-payment-monitor'),
            get_bloginfo('name'),
    ...
    private function is_premium_feature_available() {
        ...
        $is_premium = apply_filters('wc_payment_monitor_premium_available', 
