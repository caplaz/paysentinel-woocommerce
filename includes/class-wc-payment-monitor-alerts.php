...
class WC_Payment_Monitor_Alerts
{
    ...
    private function init_hooks()
    {
        ...
        add_action('wc_payment_monitor_health_calculation', [$this, 'check_all_gateway_alerts']);

        // Hook into individual health calculations
        add_action('wc_payment_monitor_gateway_health_calculated', [$this, 'check_gateway_alerts'], 10, 2);

        // Hook into gateway connectivity checks
        add_action('wc_payment_monitor_gateway_checked', [$this, 'check_gateway_connectivity_alert'], 10, 2);

        // Hook into individual payment failures for immediate critical alerts
        add_action('wc_payment_monitor_payment_failed', [$this, 'check_immediate_transaction_alert'], 10, 2);
    }
    ...
    private function get_alert_channels_for_gateway($gateway_id, $settings, $tier)
    {
        ...
        if (!empty($settings['alert_phone_number']) && in_array($tier, ['starter', 'pro', 'agency'], true)) {
            $channels[] = 'sms';
        }

        if (!empty($settings['alert_slack_workspace']) && in_array($tier, ['pro', 'agency'], true)) {
            $channels[] = 'slack';
        }
    ...
    private function is_channel_available($channel, $tier, $settings)
    {
        switch ($channel) {
            ...
            case 'sms':
                return !empty($settings['alert_phone_number']) && in_array($tier, ['starter', 'pro', 'agency'], true);

            case 'slack':
                return !empty($settings['alert_slack_workspace']) && in_array($tier, ['pro', 'agency'], true);
			...
			<div class="container">
				<div class="header">
					...
					<p><?php echo esc_html(ucfirst($alert_data['severity'])); ?>
						<?php _e('Alert', 'wc-payment-monitor'); ?></p>
				</div>
				...
				<div class="content">
					...
					<div class="stats">
						...
						<ul>
							<li><strong><?php _e('Gateway:', 'wc-payment-monitor'); ?></strong>
								<?php echo esc_html($gateway_name); ?></li>
							<li><strong><?php _e('Success Rate:', 'wc-payment-monitor'); ?></strong>
								<?php echo esc_html($success_rate); ?>%</li>
							<li><strong><?php _e('Period:', 'wc-payment-monitor'); ?></strong>
								<?php echo esc_html($period); ?></li>
							<li><strong><?php _e('Failed Transactions:', 'wc-payment-monitor'); ?></strong>
								<?php echo esc_html($failed_count); ?></li>
							<li><strong><?php _e('Total Transactions:', 'wc-payment-monitor'); ?></strong>
								<?php echo esc_html($total_count); ?></li>
							<li><strong><?php _e('Time:', 'wc-payment-monitor'); ?></strong>
    ...
    private function get_active_gateways()
    {
        ...
        $settings = get_option('wc_payment_monitor_settings', []);
        $enabled_gateways = isset($settings['enabled_gateways']) ? $settings['enabled_gateways'] : [];
    ...
    private function send_sms_notification($alert_data, $phone_number)
    {
        ...
        return $this->send_to_api($alert_data, ['sms'], $settings);
    }
    ...
    private function send_slack_notification($alert_data, $webhook_url)
    {
        ...
        return $this->send_to_api($alert_data, ['slack'], $settings);
    }
